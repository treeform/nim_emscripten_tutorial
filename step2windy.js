...
function findWasmBinary() {
  return locateFile("step2windy.wasm");
}
...
function __mmap_js(len, prot, flags, fd, offset, allocated, addr) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(2, 0, 1, len, prot, flags, fd, offset, allocated, addr);
  offset = bigintToI53Checked(offset);
  return -52;
}

function __munmap_js(addr, len, prot, flags, fd, offset) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(3, 0, 1, addr, len, prot, flags, fd, offset);
  offset = bigintToI53Checked(offset);
}
...
var proxiedFunctionTable = [ _proc_exit, exitOnMainThread, __mmap_js, __munmap_js, _fd_close, _fd_seek, _fd_write ];
